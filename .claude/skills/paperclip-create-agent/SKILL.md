...
- **process-adapter agents (scripts dispatched by Paperclip routines) require `wakeOnDemand: true` even when `heartbeat.enabled: false`** — without it, Paperclip cannot dispatch the agent and routine triggers will silently skip it. Always include `runtimeConfig: {"heartbeat": {"enabled": false, "wakeOnDemand": true}}` for any agent that will be the target of a routine.
