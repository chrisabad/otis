...
This skill is configured for the **AgentOS (AGE) production Paperclip deployment**:

- **API base:** `https://paperclip-ezk7.srv1710374.hstgr.cloud`
- **Auth:** `Authorization: Bearer $PAPERCLIP_BOARD_KEY_CLOUD` (sourced from `~/.hermes/workspace/agents/otis/.env`)
- **Web UI:** `https://paperclip-ezk7.srv1710374.hstgr.cloud/{prefix}/...`

**Company IDs:**
| Company | ID |
|---------|-----|
| AGE (AgentOS Infrastructure) | `f4593f38-24c0-481c-9771-3c52e74d16f5` |
| FON (Font Replacer) | `029fb83c-3204-4fef-a90c-85a8e89ca49d` |
| PER (Personal) | `39dc3585-54f1-4543-904e-e95d1fd9395a` |
| KAL (Kaleidoscope) | `24b4c1e0-5bc5-46bd-a391-a014ee42cab8` |

**CLI (preferred for run inspection):**
```bash
# paperclipai CLI installed at /Users/cabad/.hermes/node/bin/paperclipai
# Always pass: --api-base https://paperclip-ezk7.srv1710374.hstgr.cloud --api-key "$PAPERCLIP_BOARD_KEY_CLOUD"
/Users/cabad/.hermes/node/bin/paperclipai run list --company-id <id> --api-base https://paperclip-ezk7.srv1710374.hstgr.cloud --api-key "$PAPERCLIP_BOARD_KEY_CLOUD"
/Users/cabad/.hermes/node/bin/paperclipai run log <run-id> --api-base https://paperclip-ezk7.srv1710374.hstgr.cloud --api-key "$PAPERCLIP_BOARD_KEY_CLOUD" --text
```

**Setup at session start:**
```bash
source ~/.hermes/workspace/agents/otis/.env
# $PAPERCLIP_BOARD_KEY_CLOUD is now set
```

**Making API calls:**
```bash
source ~/.hermes/workspace/agents/otis/.env
curl -sS "https://paperclip-ezk7.srv1710374.hstgr.cloud/api/..." \
  -H "Authorization: Bearer $PAPERCLIP_BOARD_KEY_CLOUD" \
  -H "Content-Type: application/json"
```

**Agent PATCH (to pause/unpause/update):**
```bash
curl -sS -X PATCH "https://paperclip-ezk7.srv1710374.hstgr.cloud/api/agents/{id}" \
  -H "Authorization: Bearer $PAPERCLIP_BOARD_KEY_CLOUD" \
  -H "Content-Type: application/json" \
  -d '{"status": "idle"}'
```
...
**Critical rules:**
- Always `source ~/.hermes/workspace/agents/otis/.env` before API calls in interactive sessions
...
- Use the CLI for run inspection; use curl for agent/issue/approval mutations
