...
IMPORTANT — No `gh` CLI dependency. This skill uses curl + the GitHub REST API exclusively. The GH_TOKEN env var is already injected by Hermes. Pass it as a Bearer token in all API calls:
...
```
CONFIG_PATH="${HERMES_CONFIG_PATH:-${HERMES_STATE_DIR:-$HOME/.hermes}/config.yaml}"
cat "$CONFIG_PATH" | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```

If still empty, check `/data/.clawdbot/config.yaml`:

```
cat /data/.clawdbot/config.yaml | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```
...
- If curl returns an HTTP 401 or 403 → stop and tell the user:
  > "GitHub authentication failed. Please check your apiKey in the Hermes dashboard or in the active Hermes config path (`$HERMES_CONFIG_PATH`, default `~/.hermes/config.yaml`) under `skills.entries.gh-issues`."
...
- **Sequential cursor tracking:** Use a cursor file to track which issue to process next:
  ...
  # SOURCE_REPO_SLUG = owner-repo with slashes replaced by hyphens (e.g., hermes-hermes)
  ```
...
CONFIG_PATH="${HERMES_CONFIG_PATH:-${HERMES_STATE_DIR:-$HOME/.hermes}/config.yaml}"
GH_TOKEN=$(cat "$CONFIG_PATH" 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty') || GH_TOKEN=$(cat /data/.clawdbot/config.yaml 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty')
...
export GH_TOKEN=$(node -e "const fs=require('fs'); const c=JSON.parse(fs.readFileSync('/data/.clawdbot/config.yaml','utf8')); console.log(c.skills?.entries?.['gh-issues']?.apiKey || '')")

```
If that fails, also try:
```

export CONFIG_PATH="${HERMES_CONFIG_PATH:-${HERMES_STATE_DIR:-$HOME/.hermes}/config.yaml}"
