...
// src/worker.ts
var PAPERCLIP_API = process.env.PAPERCLIP_API_URL?.replace(/\/api$/, "") ?? "http://127.0.0.1:3100";
var PLUGIN_API_KEY = process.env.PAPERCLIP_BOARD_KEY ?? "";
var ROUTING_RULES_PATH = join(
...
async function getLabelId(companyId, labelName) {
  try {
    const res = await apiFetch(
      `${PAPERCLIP_API}/api/companies/${companyId}/labels`
    );
...
async function getLabelIds(companyId, labelNames) {
  ...
  try {
    ...
    if (!res.ok)
      return [];
    const data = await res.json();
    const allLabels = Array.isArray(data) ? data : data.labels ?? [];
    const nameSet = new Set(labelNames);
    return allLabels.filter((l) => nameSet.has(typeof l === "string" ? l : l.name)).map((l) => l.id).filter(Boolean);
  } catch {
    return [];
  }
}
var TRIGGER_STATUSES = /* @__PURE__ */ new Set(["todo", "triage", "unstarted"]);
var DISPATCH_WAKEUP_STATUSES = /* @__PURE__ */ new Set(["todo", "backlog"]);
var ALERT_DEDUP_WINDOW_MS = 24 * 60 * 60 * 1e3;
var ALERT_DEDUP_SHORT_WINDOW_MS = 60 * 1e3;
var ALERT_DEDUP_COMMENT_TYPES = [
  "blocked-external",
  // Generic blocked:external alert
  "circuit-breaker",
  // QA rejection circuit breaker (AGE-318)
  "approval-gatekeeper",
  // Approval label enforcement
  "approval-quality",
  // Approval quality gate
  "continuation-requeue",
  // Continuation-requeue early detection (AGE-5447)
  "dcw-circuit-breaker",
  // deferred_comment_wake circuit breaker (AGE-6126)
  "cold-queue-recovery",
  // Cold-queue recovery suppression (AGE-6413)
  "reviewer-close-recovery",
  // Reviewer close recovery sweep (AGE-6132)
  "system-timeout",
  // Paperclip system timeout auto-block
  "verdict-dedup",
  // Verdict dedup flag (AGE-6132/AGE-7277)
  "stale-execution-lock",
  // Stale execution lock auto-release (AGE-7276)
  "cross-company-assignment",
  // Cross-company agent assignment guard (AGE-7010)
  "review-gate",
  // Review gate interception (AGE-7550)
  "rate-limit",
  // Agent issue-creation rate limit (AGE-8523)
  "checkout-dedup",
  // Checkout-level dispatch dedup (AGE-7292)
  "process-adapter-migration-gate",
  // Process adapter migration script validation (AGE-12141)
  "self-verification-gate"
  // Self-verification gate for completion claims (AGE-12362)
];
var DCW_CIRCUIT_BREAKER_THRESHOLD = 3;
var DCW_CIRCUIT_WINDOW_MS = 10 * 60 * 1e3;
var DCW_COMMENT_TRIGGER_WINDOW_MS = 15 * 60 * 1e3;
var dcwSuppressionTracker = /* @__PURE__ */ new Map();
var CQR_SWEEP_INTERVAL_MS = 2 * 60 * 1e3;
var CQR_CIRCUIT_BREAKER_THRESHOLD = 3;
var CQR_CIRCUIT_WINDOW_MS = 30 * 60 * 1e3;
var cqrSuppressionTracker = /* @__PURE__ */ new Map();
var RCR_SWEEP_INTERVAL_MS = 3 * 60 * 1e3;
var RCR_VERDICT_PATTERN = /\b(PASS(?:ED|\s*\(conditional\)\b|\b)|FAIL(?:ED|\b)|REJECTED\b)/i;
var RCR_CIRCUIT_BREAKER_THRESHOLD = 3;
var RCR_CIRCUIT_WINDOW_MS = 30 * 60 * 1e3;
var STALE_EXECUTION_LOCK_THRESHOLD_MS = 30 * 60 * 1e3;
var STALE_EXECUTION_LOCK_SWEEP_INTERVAL_MS = 2 * 60 * 1e3;
var SEL_CIRCUIT_BREAKER_THRESHOLD = 3;
var SEL_CIRCUIT_WINDOW_MS = 30 * 60 * 1e3;
var selSweepTracker = /* @__PURE__ */ new Map();
var DISPATCH_DEDUP_WINDOW_MS = 60 * 1e3;
var dispatchDedupTracker = /* @__PURE__ */ new Map();
var ORCH_COMPANY_DEDUP_MS = 30 * 1e3;
var orchCompanyDedupTracker = /* @__PURE__ */ new Map();
function shouldEscalateToOrchestrator(companyId, alertType) {
  const key = companyId + ":" + alertType;
  const now = Date.now();
  const last = orchCompanyDedupTracker.get(key);
  if (last && now - last < ORCH_COMPANY_DEDUP_MS) return false;
  orchCompanyDedupTracker.set(key, now);
  return true;
}
var RECOVERY_CASCADE_MAX_DEPTH = 3;
var RECOVERY_RATE_LIMIT_PER_COMPANY_PER_24H = 5;
var RECOVERY_RATE_TRACKER = /* @__PURE__ */ new Map();
var RECOVERY_RATE_WINDOW_MS = 24 * 60 * 60 * 1e3;
var RATE_LIMIT_MAX_ISSUES_PER_HOUR = 10;
var RATE_LIMIT_MAX_SAME_PREFIX_PER_HOUR = 3;
var RATE_LIMIT_WINDOW_MS = 60 * 60 * 1e3;
var RATE_LIMIT_EXEMPT_AGENT_IDS = [
  // Healthd agent — reports real system failures, must not be rate-limited
];
async function shouldDispatchWakeup(issueId, companyId, ctx) {
  const now = Date.now();
...
function pruneDispatchDedupTracker() {
  const now = Date.now();
...
function pruneRecoveryRateTracker() {
...
var rcrSweepTracker = /* @__PURE__ */ new Map();
var alertCommentInflight = /* @__PURE__ */ new Set();
async function isDuplicateAlertComment(issueId, commentType) {
  if (!ALERT_DEDUP_COMMENT_TYPES.includes(commentType))
    return false;
  const inflightKey = `${issueId}:${commentType}`;
  if (alertCommentInflight.has(inflightKey))
    return true;
  ...
  try {
    ...
    for (let i = comments.length - 1; i >= 0; i--) {
      ...
      if (typeof c.body !== "string")
        continue;
      if (!c.body.includes(dedupTag))
        continue;
      const created = new Date(c.createdAt ?? "").getTime();
      if (!Number.isNaN(created) && now - created < ALERT_DEDUP_SHORT_WINDOW_MS) {
...
function releaseAlertCommentInflight(issueId, commentType) {
...
var plugin = definePlugin({
  async setup(ctx) {
    ...
    ctx.events.on("issue.created", async (event) => {
      ...
      const originKind = (issue?.originKind ?? "").trim();
      const isRecoveryIssue = /^Recover stalled issue\b/i.test(issueTitle) || originKind === "stranded_issue_recovery";
      if (isRecoveryIssue) {
        let isCascade = false;
        let cascadeReason = "";
        if (originKind === "stranded_issue_recovery" && issue?.parentId) {
          ...
          for (let i = 0; i < 10 && currentId && !visited.has(currentId); i++) {
            ...
            try {
              ...
              if (parent?.originKind === "stranded_issue_recovery")
                depth++;
        ...
        if (recentTimestamps.length > RECOVERY_RATE_LIMIT_PER_COMPANY_PER_24H) {
          isCascade = true;
          cascadeReason = cascadeReason ? `${cascadeReason}; rate limit ${recentTimestamps.length} > ${RECOVERY_RATE_LIMIT_PER_COMPANY_PER_24H}/24h` : `rate limit ${recentTimestamps.length} > ${RECOVERY_RATE_LIMIT_PER_COMPANY_PER_24H}/24h`;
        }
        ...
        if (isCascade) {
          ...
          if (cascadeLabelId) {
            const existingLabelIds = (issue?.labelIds ?? issue?.labels ?? []).map((l) => typeof l === "string" ? l : l?.id).filter(Boolean);
            patchBody.labelIds = [
              .../* @__PURE__ */ new Set([...existingLabelIds, cascadeLabelId])
            ];
          }
          await apiFetch(`${PAPERCLIP_API}/api/issues/${issueId}/comments`, {
            ...
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              body: `**\u{1F6D1} Recovery Cascade Guard (AGE-8553)**
...
[alert-dedup:recovery-cascade-${companyId}]`
      ...
      const createdByAgentId = issue?.createdByAgentId ?? null;
      if (createdByAgentId && !RATE_LIMIT_EXEMPT_AGENT_IDS.includes(createdByAgentId) && isConfiguredCompany(companyId)) {
        try {
          ...
          if (agentIssuesRes.ok) {
            ...
            const agentIssues = Array.isArray(agentIssuesData) ? agentIssuesData : agentIssuesData.issues ?? [];
            const recentIssues = agentIssues.filter((i) => {
              const created = new Date(i.createdAt ?? "").getTime();
              return !Number.isNaN(created) && created >= Date.now() - RATE_LIMIT_WINDOW_MS;
            });
            ...
            const samePrefixCount = recentIssues.filter(
              (i) => extractTitlePrefix((i.title ?? "").trim()) === currentTitlePrefix
            ).length;
...
[alert-dedup:rate-limit]`
      ...
      if (!issue.assigneeAgentId && !issue.assigneeUserId && status !== "done" && status !== "cancelled" && isConfiguredCompany(companyId)) {
        let targetAgentId = null;
        let assignmentSource = "";
        const AUTO_ASSIGN_EXCLUDED_AGENT_IDS = [
          "2b5f4e67-ca9a-44a2-ac1b-9ec5816d09e8"
          // Otis (COO — interactive only)
        ];
        if (issue.projectId) {
          try {
        ...
        if (targetAgentId) {
          const patchBody = { assigneeAgentId: targetAgentId };
          if (status === "backlog")
      ...
      if (!issue.parentId && status !== "done" && status !== "cancelled" && isConfiguredCompany(companyId) && issue.workMode !== "planning") {
        try {
          await apiFetch(`${PAPERCLIP_API}/api/issues/${issueId}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
      ...
      if (status !== "done" && status !== "cancelled" && isConfiguredCompany(companyId) && getAgentId(companyId, "reviewer") !== null && !issue.executionPolicy?.stages?.length) {
      ...
      if (isConfiguredCompany(companyId) && issue?.description) {
        ...
        const depRegex = /(?:\*\*Depends on:\*\*|Depends on:|Depends on)\s+(AGE-\d+)/gi;
        ...
        if (identifiers.length > 0) {
          ...
          for (const depId of identifiers) {
            try {
              ...
              if (res.ok) {
                ...
                const dep = list.find((i) => i.identifier === depId);
                if (dep) {
          ...
          if (openDeps.length > 0) {
            const existingLabelIds = (issue.labels ?? []).map((l) => typeof l === "string" ? null : l.id).filter(Boolean);
            ...
            try {
              ...
              const mergedLabelIds = [
                .../* @__PURE__ */ new Set([...existingLabelIds, ...newLabelIds])
              ];
              await apiFetch(`${PAPERCLIP_API}/api/issues/${issueId}`, {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
          ...
          const depList = depResults.map(
            (d) => `- **${d.identifier}**: \`${d.status}\` ${d.open ? "(open \u2014 blocking)" : "(done \u2014 no block)"}`
          ).join("\n");
          const commentBody = openDeps.length > 0 ? `**Auto-Dependency Detection \u2014 Issue Blocked**

Dependencies detected in issue description:
${depList}

This issue has been blocked pending resolution of the open dependencies above. It will be automatically unblocked when those issues reach \`done\` status.` : `**Auto-Dependency Detection \u2014 No Blocking Required**
...
All dependencies are already \`done\` \u2014 no blocking applied.`;
    ...
    ctx.events.on("issue.updated", async (event) => {
      ...
      const payload = event?.payload ?? {};
      const newStatus = (payload?.status ?? // Paperclip native format
      ...
      if (isConfiguredCompany(companyId)) {
        try {
          ...
          if (wakeCheckRes.ok) {
            const wakeComments = await wakeCheckRes.json() ?? [];
            if (Array.isArray(wakeComments) && wakeComments.length > 0) {
              const wakeLatest = wakeComments[wakeComments.length - 1];
              const wakeBody = typeof wakeLatest?.body === "string" ? wakeLatest.body : "";
              if (wakeBody.includes("[alert-dedup:")) {
                ctx.logger.info(
                  "AGE-7284: alert-dedup comment detected \u2014 skipping issue.updated processing to prevent wake loop",
                  {
                    ...
                    dedupTag: wakeBody.match(/\[alert-dedup:[^\]]+\]/)?.[0] ?? "unknown"
                  }
                );
                return;
              }
      ...
      if (isConfiguredCompany(companyId)) {
        try {
          ...
          if (executionRunId && executionLockedAt) {
            const lockedTime = new Date(executionLockedAt).getTime();
            const lockAgeMs = Date.now() - (Number.isNaN(lockedTime) ? 0 : lockedTime);
            if (lockAgeMs < STALE_EXECUTION_LOCK_THRESHOLD_MS) {
              if (newStatus !== "done") {
                ctx.logger.info(
      ...
      if (!newStatus)
        return;
      const previousStatus = (payload?._previous?.status ?? // Paperclip native format
      event?.changes?.status?.old ?? // legacy/fallback
      "todo").toLowerCase();
      const isTerminalReopen = (previousStatus === "done" || previousStatus === "cancelled") && (newStatus === "todo" || newStatus === "triage" || newStatus === "unstarted" || newStatus === "in_progress");
      if (isTerminalReopen && isConfiguredCompany(companyId)) {
        const dcwRevertStatus = previousStatus === "cancelled" ? "cancelled" : "done";
        try {
          const issue = await ctx.issues.get(issueId, companyId);
          ...
          if (commentsRes.ok) {
            ...
            if (Array.isArray(allComments) && allComments.length > 0) {
              ...
              const commentAge = now - new Date(latestComment?.createdAt ?? "").getTime();
              const isTriggerWindow = !Number.isNaN(commentAge) && commentAge < DCW_COMMENT_TRIGGER_WINDOW_MS;
              if (isTriggerWindow) {
                const commentAuthorAgentId = latestComment?.authorAgentId ?? null;
                const commentAuthorUserId = latestComment?.authorUserId ?? null;
                if (commentAuthorAgentId != null && assigneeAgentId != null && commentAuthorAgentId === assigneeAgentId) {
                  shouldSuppress = true;
                  suppressReason = `closing-agent-comment (authorAgentId=${commentAuthorAgentId}, age=${Math.round(commentAge / 1e3)}s)`;
                } else if (commentAuthorUserId != null && commentAuthorUserId.toLowerCase() === "local-board") {
                  shouldSuppress = true;
                  suppressReason = `local-board-proxy (authorUserId=${commentAuthorUserId})`;
                } else if (commentAuthorAgentId === null && commentAuthorUserId === null) {
                  shouldSuppress = true;
                  suppressReason = `system-comment (no authorAgentId, no authorUserId, age=${Math.round(commentAge / 1e3)}s)`;
                }
              } else {
                shouldSuppress = true;
                suppressReason = `no-comment-reopen (done\u2192${newStatus}, latest comment ${Math.round(commentAge / 1e3)}s ago exceeds ${DCW_COMMENT_TRIGGER_WINDOW_MS / 1e3}s trigger window \u2014 cold-queue recovery, AGE-6415)`;
              }
            } else {
              shouldSuppress = true;
            ...
            if (recentSuppressions >= DCW_CIRCUIT_BREAKER_THRESHOLD) {
              ...
              if (!isDupCircuit && shouldEscalateToOrchestrator(companyId, "dcw-circuit-breaker")) {
                ...
                await apiFetch(`${PAPERCLIP_API}/api/issues/${issueId}/comments`, {
                  ...
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({
                    body: `**\u{1F504} deferred_comment_wake Circuit Breaker (AGE-6126)**
        ...
        try {
          ...
          if (issue?.assigneeAgentId === reviewerAgentId) {
            ...
            try {
              ...
              if (commentsRes.ok) {
                const comments = await commentsRes.json();
                rejectionCount = Array.isArray(comments) ? comments.filter(
                  (c) => c.agentId === reviewerAgentId && /FAILED|REJECTED/i.test(c.body ?? "")
                ).length : 0;
              }
            } catch (err) {
              ctx.logger.error(
            ...
            if (rejectionCount >= 3) {
              ...
              const circuitNonBlockedLabelNames = (issue.labels ?? []).map((l) => typeof l === "string" ? l : l.name).filter((n) => !n.startsWith("blocked:"));
              ...
              const circuitMergedLabelIds = externalLabelId ? [.../* @__PURE__ */ new Set([...circuitLabelIds, externalLabelId])] : circuitLabelIds;
              await apiFetch(`${PAPERCLIP_API}/api/issues/${issueId}`, {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
          ...
          if (isGateCheck) {
            ...
            if (!hasEvidence) {
              ...
              await apiFetch(`${PAPERCLIP_API}/api/issues/${issueId}/comments`, {
                ...
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                  body: `**\u{1F50D} Gate-Check Evidence Required (AGE-6129)**
...
**Rule (AGE-6129):** LLM must judge evidence, not generate it. The skill runs queries deterministically; Juno reads the output and marks each criterion PASS/FAIL/ERROR.`
      ...
      async function extractClaimsFromComment(body) {
        const claims = [];
        ...
        if (commits) {
          for (const match of commits) {
            ...
            if (shaMatch)
              claims.push({ claim: shaMatch[0], type: "git" });
          }
        }
        ...
        for (const m of commitLabels) {
          if (m[1])
            claims.push({ claim: m[1], type: "git" });
        }
        ...
        if (prUrls) {
          for (const url of prUrls) {
            if (!claims.some((c) => c.claim === url && c.type === "github_pr")) {
        ...
        for (const m of branchMatches) {
          ...
          if (branchName && branchName.length > 1) {
            if (!claims.some((c) => c.claim === branchName && c.type === "branch")) {
      ...
      async function verifyFilePath(path2) {
        try {
          const { execSync: execSync2 } = __require("node:child_process");
          execSync2(`test -e "${path2}"`, { stdio: "pipe" });
          return null;
        } catch {
          return {
            claim: path2,
      ...
      async function verifyGitCommit(sha) {
        try {
          const { execSync: execSync2 } = __require("node:child_process");
      ...
      async function verifyGitHubPR(prUrl) {
        try {
          const { execSync: execSync2 } = __require("node:child_process");
          ...
          if (msg.includes("not found") || msg.includes("404") || msg.includes("no pull request")) {
            return {
              claim: prUrl,
              type: "pr",
      ...
      async function verifyBranchName(branchName, repoHints) {
        const { execSync: execSync2 } = __require("node:child_process");
        const KNOWN_REPOS = [
          "openclaw",
          "openclaw-llm-proxy",
        ...
        for (const repo of reposToCheck) {
          try {
            ...
            if (msg.includes("could not resolve host") || msg.includes("authentication token") || msg.includes("bad credentials")) {
              return null;
            }
          }
        }
      ...
      async function verifyCompletion(failures) {
      ...
      if (newStatus === "done" && isConfiguredCompany(companyId)) {
        try {
          ...
          const isMigration = /Process.*routine|Migrate.*cron|Migrate.*[Pp]rocess/i.test(title) || /adapterType:\s*process|Process routine|process.*adapter/i.test(
            description
          ...
          if (isMigration) {
            ...
            const hasRuntimeOnlyAck = runtimeOnlyMatch && runtimeOnlyMatch[1]?.trim().length > 20;
            if (hasRuntimeOnlyAck) {
              ...
              const scriptPathRegex = /(?:tools\/|scripts\/|\.\/)[^\s\)]+\.(?:py|sh|js|mjs|ts)/gi;
              const scriptPaths = Array.from(
                description.matchAll(scriptPathRegex)
              ).map((m) => m[0]);
              if (scriptPaths.length > 0) {
                const uncommittedScripts = [];
                const REPO_ROOTS = [
                ...
                if (uncommittedScripts.length > 0) {
                  ...
                  const commentBody = `**\u{1F512} Process Adapter Migration Gate (AGE-12141)**
          ...
          if (lastComment) {
            ...
            if (claims.length > 0) {
              ...
              if (failures.length > 0) {
                ...
                const failuresList = failures.map((f) => `- \u274C ${f.claim} (${f.type}): ${f.reason}`).join("\n");
...
[alert-dedup:self-verification-gate]`;
      ...
      if (newStatus === "done" && isConfiguredCompany(companyId)) {
        ...
        const blockedByIdentifiers = Array.isArray(issue?.blockedBy) ? issue.blockedBy.filter((id) => typeof id === "string") : [];
        const blocksIdentifiers = Array.isArray(issue?.blocks) ? issue.blocks.filter((id) => typeof id === "string") : [];
        const blockingLabelsFromRelationships = [
          ...blockedByIdentifiers.map((id) => `blocked:issue-${id}`),
          ...blocksIdentifiers.map((id) => `blocked:issue-${id}`)
        ];
        const allBlockingLabels = /* @__PURE__ */ new Set([
          blockingLabel,
        ...
        for (const scanCompanyId of companyIds) {
          try {
            ...
            if (!Array.isArray(dependentIssues))
              continue;
            for (const dependent of dependentIssues) {
              const matchingLabels = [];
              const dependentLabelNames = (dependent.labels || []).map((l) => typeof l === "string" ? l : l?.name);
              for (const label of allBlockingLabels) {
                if (dependentLabelNames.includes(label)) {
                  matchingLabels.push(label);
                }
              }
              const dependentBlockedBy = Array.isArray(
                dependent.blockedBy
              ) ? dependent.blockedBy.filter(
                (id) => typeof id === "string"
              ) : [];
              const hasRelationshipRef = dependentBlockedBy.includes(blockingIssueId) || dependentBlockedBy.includes(blockingIdentifier);
              const hasBlocksRef = blocksIdentifiers.includes(dependent.id) || dependent.identifier != null && blocksIdentifiers.includes(dependent.identifier);
              const isDependent = matchingLabels.length > 0 || hasRelationshipRef || hasBlocksRef;
              if (!isDependent)
                continue;
              try {
                ...
                const targetAssignee = dependent.assigneeAgentId || getAgentId(scanCompanyId, "implementer");
                const remainingLabelNames = remainingLabels.map(
                ...
                const blockerDescription = matchingLabels.length > 0 ? matchingLabels.join(", ") : `${blockingIdentifier} (via blockedBy/blocks relationship)`;
                const commentBody = stillBlocked ? `**Auto-Unblock (Partial)**

${blockingIdentifier} resolved, removing ${blockerDescription}. Issue remains blocked by: ${remainingBlockerLabels.map((l) => typeof l === "string" ? l : l?.name).join(", ")}.

[auto-unblock:${blockingIdentifier}]` : `**Auto-Unblock**
...
[auto-unblock:${blockingIdentifier}]`;
    ...
    ctx.events.on("approval.decided", async (event) => {
      const companyId = event?.companyId ?? "";
      if (!companyId || !isConfiguredCompany(companyId))
        return;
      const payload = event?.payload ?? {};
      ...
      if (approvalType !== "structural_change")
        return;
      ...
      for (const issueId of issueIds) {
        try {
          const issue = await ctx.issues.get(issueId, companyId);
          const labels = issue?.labels ?? [];
          const hasApprovalLabel = labels.some(
            (label) => typeof label === "string" ? label === "blocked:needs-approval" : label.name === "blocked:needs-approval"
          );
          if (!hasApprovalLabel) {
            ctx.logger.info(
          ...
          const targetAssignee = issue?.assigneeAgentId || getAgentId(companyId, "implementer");
          const approvalLabelNames = updatedLabels.map(
            (l) => typeof l === "string" ? l : l.name
          );
          ...
          const commentBody = approvalStatus === "approved" ? `**Approval Granted \u2014 Ready for Work**
...
This issue has been unblocked and is ready to proceed. Implementation can resume immediately.` : `**Approval Rejected \u2014 Review Required**
      ...
      for (const [companyId, companyCfg] of Object.entries(
        ...
        try {
          ...
          for (const issue of issues) {
            ...
            const now = Date.now();
            const existingTs = (cqrSuppressionTracker.get(issueId) ?? []).filter((ts) => now - ts < CQR_CIRCUIT_WINDOW_MS);
            ...
            if (!isDupAlert && shouldEscalateToOrchestrator(companyId, "cold-queue-recovery")) {
              ...
              await apiFetch(`${PAPERCLIP_API}/api/issues/${issueId}/comments`, {
                ...
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                  body: `**\u{1F504} Cold-Queue Recovery Suppressed (AGE-6415)**
...
[alert-dedup:cold-queue-recovery]`
    ...
    const STALE_AGENT_THRESHOLD_MS = 15 * 60 * 1e3;
    const STALE_AGENT_WATCHDOG_INTERVAL_MS = 5 * 60 * 1e3;
      ...
      for (const [companyId, companyCfg] of Object.entries(
        ...
        try {
          ...
          for (const agent of agents) {
            ...
            const lastHeartbeatAt = agent.lastHeartbeatAt ? new Date(agent.lastHeartbeatAt).getTime() : 0;
            const staleForMs = Date.now() - lastHeartbeatAt;
            if (staleForMs < STALE_AGENT_THRESHOLD_MS)
              continue;
    ...
    const COLD_QUEUE_SWEEP_INTERVAL_MS = 3 * 60 * 1e3;
    const COLD_QUEUE_DISPATCHABLE_STATUSES = /* @__PURE__ */ new Set(["todo", "backlog"]);
    const coldQueueSweepTracker = /* @__PURE__ */ new Map();
    const COLD_QUEUE_CIRCUIT_BREAKER_THRESHOLD = 3;
    const COLD_QUEUE_CIRCUIT_WINDOW_MS = 30 * 60 * 1e3;
    ...
    const XCA_CIRCUIT_WINDOW_MS = 30 * 60 * 1e3;
    const XCA_SWEEP_INTERVAL_MS = 10 * 60 * 1e3;
    const xcaSweepTracker = /* @__PURE__ */ new Map();
    async function buildAgentCompanyMap() {
      ...
      for (const [companyId] of Object.entries(routingConfig.companies)) {
        try {
          ...
          for (const issue of issues) {
            ...
            if (!agentCompanyId)
              continue;
            if (agentCompanyId === issueCompanyId)
              continue;
            const issueId = issue.id;
            const identifier = issue.identifier ?? issueId;
            ...
            if (!isDupAlert && shouldEscalateToOrchestrator(issueCompanyId, "xca-circuit-breaker")) {
              ...
              await apiFetch(`${PAPERCLIP_API}/api/issues/${issueId}/comments`, {
                ...
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                  body: `**\u{1F500} Cross-Company Assignment Detected (AGE-7010)**
        ...
        try {
          ...
          issues = issues.filter((issue) => {
            ...
            if (status === "blocked") {
              const labels = issue.labels ?? [];
              return labels.some(
                (l) => typeof l === "string" ? l === "blocked:external" : l.name === "blocked:external"
              );
            }
            return false;
          });
          for (const issue of issues) {
            ...
            try {
              ...
              if (!Array.isArray(allComments) || allComments.length === 0)
                continue;
              const reviewerVerdicts = allComments.filter(
                (c) => c.authorAgentId === reviewerAgentId && typeof c.body === "string" && RCR_VERDICT_PATTERN.test(c.body)
              ).sort(
                (a, b) => new Date(b.createdAt ?? 0).getTime() - new Date(a.createdAt ?? 0).getTime()
              );
              if (reviewerVerdicts.length === 0)
                continue;
              ...
              if (!isDupAlert) {
                await apiFetch(`${PAPERCLIP_API}/api/issues/${issueId}/comments`, {
                  ...
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({
                    body: `**\u{1F504} Reviewer Close Recovery (AGE-6132)**
          ...
          for (const issue of issues) {
            ...
            if (!isDupAlert && shouldEscalateToOrchestrator(companyId, "stale-lock-release")) {
              ...
              await apiFetch(`${PAPERCLIP_API}/api/issues/${issueId}/comments`, {
                ...
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                  body: `**\u{1F513} Stale Execution Lock Auto-Released (AGE-7276)**

The execution lock on this issue was held for **${Math.round(lockAgeMs / 6e4)} minutes** (threshold: 30 min) by run \`${executionRunId}\` without producing output. The lock has been auto-released so subsequent runs can proceed.
...
[alert-dedup:stale-execution-lock]`
    ...
    if (staleAgentInitial.unref)
      staleAgentInitial.unref();
    const staleAgentInterval = setInterval(() => {
      void sweepStaleAgents();
    }, STALE_AGENT_WATCHDOG_INTERVAL_MS);
    if (staleAgentInterval.unref)
    ...
    if (xcaInterval.unref)
      xcaInterval.unref();
    const coldQueueInitial = setTimeout(() => {
      void sweepColdQueue();
    }, 45e3);
    if (coldQueueInitial.unref)
      coldQueueInitial.unref();
    const coldQueueInterval = setInterval(() => {
      void sweepColdQueue();
    }, COLD_QUEUE_SWEEP_INTERVAL_MS);
    if (coldQueueInterval.unref)
      coldQueueInterval.unref();
    const dedupPruneInterval = setInterval(() => {
      pruneDispatchDedupTracker();
      pruneRecoveryRateTracker();
      (function pruneOrchCompanyDedupTracker() { const now = Date.now(); for (const [k, ts] of orchCompanyDedupTracker) { if (now - ts >= ORCH_COMPANY_DEDUP_MS) orchCompanyDedupTracker.delete(k); } })();
      (function pruneColdQueueTracker() {
        const now = Date.now();
        for (const [issueId, timestamps] of coldQueueSweepTracker) {
          const filtered = timestamps.filter((ts) => now - ts < COLD_QUEUE_CIRCUIT_WINDOW_MS);
          if (filtered.length === 0) {
            coldQueueSweepTracker.delete(issueId);
          } else {
            coldQueueSweepTracker.set(issueId, filtered);
          }
        }
      })();
    }, 60 * 1e3);
    if (dedupPruneInterval.unref)
      dedupPruneInterval.unref();
    ctx.logger.info(
      "Issue Trigger v1.44.0+age114 \u2014 event-routing + cold-queue dispatch sweep (AGE-114). Recovery cascade guard (AGE-8553): depth limit (3 ancestors) + per-company rate limit (5/24h) for stranded_issue_recovery issues, blocked:cascade label for observability. Checkout-level dispatch dedup (AGE-7292). Stale execution lock sweep + concurrent run guard (AGE-7276). Review gate (AGE-7550). Agent issue-creation rate limit (AGE-8523). Cold-queue dispatch sweep for assigned-but-never-started issues (AGE-114)."
    );
  },
  async onHealth() {
    return {
      status: "ok",
      message: "v1.44.0+age114 \u2014 Event-routing + cold-queue dispatch sweep (AGE-114). No drain loop. Paperclip native heartbeat + executionPolicy handle wakeup and review. Dispatcher wakeup gated on dispatcher-agent presence (AGE-691). A.4 executionPolicy auto-apply on issue.created. Plan-first demotion (planRequired). Blocked-label validation. Auto-dependency detection (AGE-306). QA rejection circuit breaker (AGE-318). Recovery cascade guard (AGE-8553). Checkout-level dispatch dedup (AGE-7292). Retry storm breaker (AGE-5123). Continuation-requeue (AGE-5447). Alert dedup (AGE-5261). Dependency-aware auto-unblock (AGE-6080). Gate-check evidence enforcement (AGE-6129). Stale execution lock sweep (AGE-7276). Agent issue-creation rate limit (AGE-8523). Cold-queue dispatch sweep for assigned-but-never-started issues (AGE-114)."
    };
  }
});
